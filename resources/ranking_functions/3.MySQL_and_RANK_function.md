...
SELECT
      val,
      RANK()       OVER (ORDER BY val) the_rank,
      DENSE_RANK() OVER (ORDER BY val) the_dense_rank,
      ROW_NUMBER() OVER (ORDER BY val) the_row_number
FROM
t;
+------+----------+----------------+----------------+
| val  | the_rank | the_dense_rank | the_row_number |
+------+----------+----------------+----------------+
|    1 |        1 |              1 |              1 |
|    2 |        2 |              2 |              2 |
|    2 |        2 |              2 |              3 |
...
## Find top performer employee per year
...
## Find bottom performer employee per year
