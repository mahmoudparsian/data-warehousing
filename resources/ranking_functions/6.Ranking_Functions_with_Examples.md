...
* <mark>Leaderboards:</mark> Rank players or teams based on their performance.

* <mark>Top-N Analysis:</mark> Identify the top-performing products, customers, or employees.

* <mark>Sales Analysis:</mark> Rank sales representatives by their sales volume.

* <mark>Customer Segmentation:</mark> Rank customers based on their purchase frequency or total spending.

* <mark>Performance Evaluation:</mark> Rank employees based on their performance metrics.
...
* ✅ `ROW_NUMBER()` Sequential numbers: 1, 2, 3, 4, ...
  ...
  | <mark>score</mark> | `row_number()` |`rank()`| `dense_rank()` |
  |-------|----------------|--------|----------------|
  |  10   |  1             | 1      | 1              |
  |  11   |  2             | 2      | 2              |
  |  11   |  3             | 2      | 2              |
...
~~~sql
DENSE_RANK() OVER (PARTITION BY partition_column 
                   ORDER BY order_column ASC/DESC)
~~~
...
Subject_Ranks AS (
    ...
    FROM 
        ExamResult er
    JOIN 
        Subject_Averages sa ON er.subject = sa.subject
)
SELECT 
...
### Query Using `RANK()` with Partitioning by Multiple Columns
